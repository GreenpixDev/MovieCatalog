...
class MainViewModel : ViewModel() {
    ...
    private val _favoritesState = mutableStateMapOf<Int, MainFavorite>()
    private val _galleryState = mutableStateMapOf<Int, MainMovie>()

    val loadState: State<LoadState>
        get() = _loadState
    val favoritesState: Map<Int, MainFavorite>
        get() = _favoritesState
    val galleryState: Map<Int, MainMovie>
        get() = _galleryState
    ...
    suspend fun load() {
        ...
        delay(1000)
        for (i in 1..10) {
            _favoritesState[i] = MainFavorite(exampleImageUrl)
        }
        for (i in 1..10) {
            _galleryState[i] = MainMovie(
    ...
    fun onDeleteFavorite(movieId: Int) {
